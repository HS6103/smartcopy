...
"""
    Loki module for foreign_namesTW
...
MODULE_DICT = {
    "Account": import_from_path("smartcopy_TW_lib_Account", os.path.join(os.path.dirname(CWD_PATH), "lib/Account.py")),
    "LLM": import_from_path("smartcopy_TW_lib_LLM", os.path.join(os.path.dirname(CWD_PATH), "lib/LLM.py"))
}
"""
Account 變數清單
[變數] BASE_PATH         => 根目錄位置
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "祁凱立（Kharis Templeman）":
        if CHATBOT:
            ...
            entity_text = ' '.join(s.strip() for s in re.findall(r"<ENTITY_oov>([^<]+)</ENTITY_oov>", args[1]))
            resultDICT["en_name"].append((args[0], entity_text))
...
if __name__ == "__main__":
    ...
    refDICT={"en_entity": []}
    resultDICT = getResult("「虔誠軍」（Lashkar-e-Taiba, LET）", "「虔誠軍」（Lashkar-e-Taiba, LET）", refDICT={"en_entity": []}, args=[], resultDICT={})
    pprint(resultDICT)